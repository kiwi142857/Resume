...
\usepackage{amsmath}   % 添加对\text命令的支持
...
{\color{darkgray}\faEnvelope} & qiweic10@sjtu.edu.cn & {\color{darkgray}\faUser} 中共党员 \\
...
\vspace{-2em}
\section*{教育背景}
\noindent\textbf{\large 上海交通大学} \textbf{软件工程专业} \hfill 2022.08 - 至今\\
\vspace{1em}
\textit{学积分: 91.1/100 (专业排名: 8/89)} \hfill \textit{GPA: 3.92/4.30 (专业排名: 9/89)}\\
...
\item 85届计算机系教育发展基金暨杨元庆教育基金奖学金 \hfill 2024-2025学年
\item 中国国际大学生创新大赛金奖（上海赛区） \hfill 2024-2025学年
\end{itemize}

\section*{项目经历}
\begin{itemize}[leftmargin=*,label={},itemsep=0.5em,topsep=0.2em]

% \item \textbf{vLLM框架下大模型本地部署} \hfill \textit{2025.03 - 2025.03}\\
% 负责在Atlas 800I A2集群，8 $\times$ Ascend 910b（2$\times$8$\times$64G）上使用vLLM框架容器化部署大模型DeepSeek-671b-W8A8 并调优。实现了单请求下17$\text{ tokens/s}$的推理速度，256并发下$\text{throughput}$达到1280 $\text{ tokens/s}$。

\item \textbf{{ICS 课程实验优化}} \hfill \textit{2024.08 - 2025.05}\\
设计并实现 LLMLab 实验模块，针对 \textit{Transformer} 架构的模型进行 并行优化与 KV Cache 优化实验，并制定相应的评测方案。负责课程实验平台迁移（SVN → Git），构建在线评测查分系统。
\item \textbf{VLA，VLM等模型在国产端侧设备的部署，推理优化} \hfill \textit{2025.05 - 2025.08}\\
负责 VLA、VLM、SAM2 等模型 在 Ascend 310P、瑞芯微 RK3588 等国产端侧芯片上的适配，部署与推理优化。在RK3588上基于 OpenHarmony（国产操作系统） 实现算子性能优化与内存占用。将基于 llama.cpp 的 VLM 模型优化方案 提交并合并至开源社区，参与开源社区贡献。
...
担任{\href{https://github.com/UniGPT-SJTU}{项目}}组长，带领4人团队进行敏捷开发。负责系统架构设计和技术选型，实现基于k8s的服务编排和自动化部署流程。通过langchain4j框架集成大模型功能，实现智能提示词推荐。项目获学院软件展示会大二组优胜奖。
...
负责基于OpenTrustee和llama.cpp实现大模型在端侧设备的安全推理部署。设计并实现TEE安全环境下的模型加载和推理模块，通过硬件加密保护模型参数安全。针对性能瓶颈进行优化。项目获OpenHarmony竞赛训练营全国三等奖。
...
% \item \textbf{\href{https://github.com/kiwi142857/Minik8s}{Minik8s}} \hfill \textit{2025.03 - 2025.06}\\
\item \textbf{{Minik8s}} \hfill \textit{2025.03 - 2025.06}\\
担任\textit{Minik8s}项目组长，与团队共同开发实现简化版k8s集群调度编排系统，实现pod调度，服务发现，负载均衡等功能。
...
% \item \textbf{开发技能}：微服务架构设计、DevOps、大模型部署优化、安全计算、分布式系统、llama.cpp，vLLM容器化部署
